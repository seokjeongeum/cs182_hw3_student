...
class PositionEmbedding(nn.Module):
    ...
    def __init__(self, hidden_size):
        ...
        assert (
            hidden_size % 2 == 0
            and "Model vector size must be even for sinusoidal encoding"
        )
    ...
    def forward(self, inputs, start=1):
        ...
        assert (
            inputs.shape[-1] == self.hidden_size
            and "Input final dim must match model hidden size"
        )
        ...
        position_embedding = th.stack((sin_embedding, cos_embedding), dim=3).view(
            position_shape
        )
...
class TransformerFeedForward(nn.Module):
    def __init__(self, input_size, filter_size, hidden_size, dropout):
...
class TransformerDecoderBlock(nn.Module):
    ...
    def forward(
        ...
        target_selfattn = self.self_attention(
            [norm_decoder_inputs, norm_decoder_inputs], self_attention_mask
        )
        ...
        encdec_attention = self.cross_attention(
            [norm_target_selfattn, norm_encoder_outputs], cross_attention_mask
        )
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(
        self, embedding_layer, n_layers, n_heads, d_model, d_filter, dropout=None
    ):
        super().__init__()
        ...
        for i in range(n_layers):
            encoder = TransformerEncoderBlock(
                embed_size, n_heads, d_filter, d_model, dropout
            )
...
class TransformerDecoder(nn.Module):
    ...
    def shift_target_sequence_right(self, target_sequence):
        ...
        pad_array = [1, 0, 0, 0]
        target_sequence = F.pad(target_sequence, pad_array, value=constant_values)[
            :, :-1
        ]
    ...
    def get_self_attention_mask(
        self, batch_size, sequence_length, decoder_mask, mask_future
    ):
        if not mask_future:
            return decoder_mask
    ...
    def get_cross_attention_mask(
        self, encoder_output, decoder_input, encoder_mask, decoder_mask
    ):
        if encoder_mask is None and decoder_mask is None:
            ...
            cross_attention_mask = (
                decoder_mask[:, 1, :][:, None, :]
                .repeat(*(1, encoder_output.shape[1], 1))
                .permute((0, 2, 1))
            )
        elif decoder_mask is None:
            cross_attention_mask = (
                encoder_mask[:, 1, :][:, :, None]
                .repeat(*(1, 1, decoder_input.shape[1]))
                .permute((0, 2, 1))
            )
        else:
            dec_attention_mask = (
            ...
            cross_attention_mask = th.logical_and(
...
class Transformer(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.encoder = TransformerEncoder(
            input_embedding, n_layers, n_heads, d_model, d_filter, dropout
        )

        # Build the decoder stack.
        self.decoder = TransformerDecoder(
            input_embedding, output_layer, n_layers, n_heads, d_model, d_filter, dropout
        )
    ...
    def forward(
        ...
        # mask shapes.
        encoder_mask = transformer_utils.convert_to_attention_mask(
            source_sequence, encoder_mask
        )
        decoder_mask = transformer_utils.convert_to_attention_mask(

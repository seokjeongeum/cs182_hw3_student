...
class AttentionQKV(nn.Module):
    ...
    def forward(self, queries, keys, values, mask=None):
        ...
        similarity = (
            queries @ keys.transpose(-1, -2) / th.sqrt(key_dim)
...
class MultiHeadProjection(nn.Module):
    def __init__(self, n_heads, feature_sizes):
        ...
        for size in feature_sizes:
            assert (
                size % self.n_heads == 0
            ), "Shape of feature input must be divisible by n_heads"
    ...
    def _combine_heads(self, tensor):
        ...
        tensor = tensor.reshape(
            batch_size, tensorlen, new_feature_size
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_heads, input_shapes):
        ...
        self.attention_layer = MultiHeadProjection(
            n_heads, (self.qa_channels, self.ma_channels)
        )

        assert (
            self.qa_channels % self.n_heads == 0
            and self.ma_channels % self.n_heads == 0
            and "Feature size must be divisible by n_heads"
        )
        assert (
            self.qa_channels == self.ma_channels
            and "Cannot combine tensors with different shapes"
        )

        self.query_layer = weight_norm(
            nn.Linear(self.qa_channels, self.qa_channels, bias=False)
        )
        self.key_layer = weight_norm(
            nn.Linear(self.qa_channels, self.qa_channels, bias=False)
        )
        self.value_layer = weight_norm(
            nn.Linear(self.ma_channels, self.ma_channels, bias=False)
        )
    ...
    def forward(self, inputs, mask=None):
        ...
        """
        assert (
            (isinstance(inputs, tuple) or isinstance(inputs, list))
            and len(inputs) == 2
            and "Must pass query and memory"
        )
